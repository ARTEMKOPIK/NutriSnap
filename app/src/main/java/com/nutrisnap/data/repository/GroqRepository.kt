...
class GroqRepository(private val apiService: GroqApiService, private val apiKey: String) {
    ...
    suspend fun analyze(text: String?, imageBytes: ByteArray?): Result<FoodAnalysis> = withContext(Dispatchers.IO) {
        try {
            val contentList = mutableListOf<Content>()
            
            // Choose model based on input type
            val modelId = if (imageBytes != null) {
            ...
            val jsonContent = response.choices.firstOrNull()?.message?.content ?: return@withContext Result.failure(Exception("Empty response"))
